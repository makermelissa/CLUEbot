...
class Robot:
    def __init__(self, left_pin, right_pin, underlight_neopixel):
        self.left_motor = self.init_motor(left_pin)
        self.right_motor = self.init_motor(right_pin)
        self.init_display()
        self.init_ble()
    ...
    def rotate_right(self):
        ...
        self.set_status_rotate_cw() if self.direction == STOP else self.set_status_right()
        speed = FWD if self.direction == STOP else self.direction
        self.set_left_throttle(speed)
        self.set_right_throttle(STOP if self.direction != STOP else -1 * speed)

    def rotate_left(self):
        self.release_color = self.get_underglow()
        self.set_underglow(YELLOW, True)
        self.set_status_rotate_ccw() if self.direction == STOP else self.set_status_left()
        speed = FWD if self.direction == STOP else self.direction
        self.set_left_throttle(STOP if self.direction != STOP else -1 * speed)
    ...
    def set_status_stop(self):
        self.remove_shapes()
        self.add_centered_rect(100, 100)
